...
function FoodLogCard({ itemObj, viewType, onUpdate }) {
  ...
  const handleDishMouseEnter = () => {
    setHoveredItem(itemObj.dish);
  ...
  const handleRestaurantMouseEnter = () => {
    setHoveredItem(itemObj.restaurant);
  ...
  return (
    <Card
      className="card"
      style={{ width: '18rem', margin: '10px', border: '1px solid' }}
      onMouseEnter={handleDishMouseEnter}
