...
async function seedData(uid) {
  try {
    ...
    for (const restaurant of restaurantsData) {
      ...
      payload.uid = uid; // Use provided UID
    ...
    for (const log of foodlogsData) {
      const { id, restaurant_id, dish_id, category_ids, uid: _oldUid, ...payload } = log;
...
if (!uid) {
  ...
  console.error('Please provide the current Firebase authenticated user\'s UID as an argument.');
  process.exit(1);
}
