  ...
  return (
    <Card className="card" style={{ width: '18rem', margin: '10px', border: '1px solid' }}>
      ...
      <Card.Body>
        ...
        <p className="card-text bold">Category: {itemObj.category.map((category) => category.category_name).join(', ')}</p>
        ...
        <pre>{JSON.stringify(itemObj, null, 2)}</pre> {/* Display the entire itemObj */}
      </Card.Body>
    </Card>
  );
}
